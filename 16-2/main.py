...
def split_beam(beams, row, col, beam):
    beams.remove(beam)
    if grid[row][col] == "|":
...
def move_next(beams, energized_tiles, beam):
    ...
    if next_tile in ["/", "\\"]:
        ...
        split_beam(beams, next_pos[0], next_pos[1], beam)
...
starting_beams = []
for row in range(0, len(grid)):
    starting_beams.append(Beam(row, -1, "left"))
    starting_beams.append(Beam(row, len(grid[0]), "right"))
for col in range(0, len(grid[0])):
    starting_beams.append(Beam(-1, col, "down"))
    starting_beams.append(Beam(len(grid), col, "up"))
...
for starting_beam in starting_beams:
    ...
    while len(beams) != 0:
        for beam in beams.copy():
            ...
            move_next(beams, energized_tiles, beam)

    if len(energized_tiles) > max_energized:
        max_energized = len(energized_tiles)
